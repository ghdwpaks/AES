...
import copy as c
import enc_KeyScheduling as ks
    ...
    def RollBack_apply(w,sbox) :
        ...
        for i in range(len(sbox)) :
            if w in sbox[i] :
                r = sbox[i].index(w)
                c = i
        r, c = tr.str_int(r),tr.str_int(c)

        return "".join([str(c),str(r)])

    def Rollback_rot(key_part,rot=1) :
        #print("Rollback_rot rot :",rot)
        ...
        temp = []
        for i in range(rot) :
            #print("res appending :",key_part[-(i+1)])
            res.append(key_part[-(i+1)])
        res.reverse()
        for i in range(4-rot) :
            #print("temp appending :",key_part[i])
